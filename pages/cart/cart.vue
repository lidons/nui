...
<template name="glanceShopcart">
	...
	>
		...
		<view v-if="shownullcart" style="width: 100%;height: 250px;">
			...
			<view
				style="height: 30px;width: 100%;font-size: 15px;line-height: 30px;text-align: center;"
			>
				您的购物车为空，快去添加心爱的商品吧！
			</view>
		</view>
		...
		<view v-for="(thiscart, index) in cart" :key="index" style="background-color: #FFFFFF;">
			...
			<view
				...
				v-if="thiscart.groupkey && cntitems > 0"
				@click="clickgroupkey(thiscart.groupkey)"
			>
				{{ thiscart.groupkey }}
			</view>
			<!-- 按营销活动分组的商品 -->
						...
						<view class="glance-shop-cart-scrollx-items-item">
							...
							<view style="width: 10%;height: 100%;background-color: #FFFFFF;">
								...
								<view
									...
									@click="clickitemselected(item.id)"
								></view>
							</view>
							<!-- 图片 -->
							...
							<view class="glance-shop-cart-items-item-des">
								...
								<view class="glance-shop-cart-items-item-pq">
									...
									<view class="glance-shop-cart-items-item-opt">
										<!-- 减数量 -->
										<view
											class="glance-shop-cart-items-item-opt-quantity-minus"
											@click="minusitem(item.id)"
										>
											-
										</view>
										<!-- 数量 -->
										<view class="glance-shop-cart-items-item-opt-quantity">
											{{ item.quantity }}
										</view>
										<!-- 加数量 -->
										<view
											class="glance-shop-cart-items-item-opt-quantity-plus"
		...
		<!-- 购买此商品的人还买了 -->
		<view style="height: 750px;background-color: #FFFFFF;" v-if="cntitems > 0">
			购买此商品的人还买了
		</view>
		<view style="height: 10px;background-color: #F5F5F5;"></view>
		<!-- 猜你喜欢 -->
	...
	onLoad: function() {
		...
		for (let i = 0; i < this.cart.length; i++) {
			for (let k = 0; k < this.cart[i].items.length; k++) {
				// 总金额
				this.totalamount =
					this.totalamount + this.cart[i].items[k].price * this.cart[i].items[k].quantity;
				// 总数量
					...
					for (var i = 0; i < this.cart.length; i++) {
						for (let k = 0; k < this.cart[i].items.length; k++) {
							...
							if (this.cart[i].items[k].id < 0) {
								...
								this.totalamount =
									this.totalamount +
									this.cart[i].items[k].price * this.cart[i].items[k].quantity;
								this.totalamount = this.fmamount(this.totalamount);
							}
						}
					}
		...
		plusitem(itemid) {
			for (let i = 0; i < this.cart.length; i++) {
				for (let k = 0; k < this.cart[i].items.length; k++) {
					...
					if (
						this.cart[i].items[k].id == itemid &&
						this.cart[i].items[k].quantity < 100
					) {
						// 更新item数量
						this.cart[i].items[k].quantity = this.cart[i].items[k].quantity + 1;
						// 勾选状态下更新数量和金额
